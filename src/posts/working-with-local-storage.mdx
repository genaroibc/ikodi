...
- `JSON.parse()` accepts a valid **stringified** JavaScript
  value and returns a its equivalent JavaScript value:
...
We create a _select_ element with language options and we add it the "language-select" _id_.
...
```javascript
// we listen to "change" events in the document
document.addEventListener("change", e => {
  // we check if it comes from our select element
  if (e.target.matches("#language-select")) {
    // we stringify the selected value
    const stringifiedValue = JSON.stringify(e.target.value);

    // we save it in the Local Storage
    localStorage.setItem("language", stringifiedValue);
  }
});
```
...
> \* When i work with vanila JavaScript, i add a single event listener for each event i need to handle. I do it in order to prevent _event propagation_ and reduce the number of event listeners in the app.

Once we've added our event listener, we check if the event target (the element that has triggered the event) is our _select_. If so, we get its value (in our example it would be _en_, _es_ or _pt_) and save it in the Local Storage.

Note that we are converting our value to a string before saving it.
